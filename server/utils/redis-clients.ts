...
const adapterPubClient = new RedisClient({
  host:
    process.env.NAME === 'willThisWork' ? 'redisSocketIoAdapter' : 'localhost',
  port: 6379,
});
...
const roomStateClient = new RedisClient({
  host: process.env.NAME === 'willThisWork' ? 'redisRoomState' : 'localhost',
  port: 6380,
});
...
const clientRoomIdClient = new RedisClient({
  host: process.env.NAME === 'willThisWork' ? 'redisClientRoomId' : 'localhost',
...
const waitingRoomIdClient = new RedisClient({
  host:
    process.env.NAME === 'willThisWork' ? 'redisWaitingRoomId' : 'localhost',
